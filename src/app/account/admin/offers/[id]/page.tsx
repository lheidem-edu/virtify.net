...
import { loadContract, loadOffer } from "@/lib/documents/repository";
import {
...
export default async function Page({
    ...
    const loaded = await loadOffer(id, session.user.id, true);
    ...
    const contract = offer.contractId
        ? await loadContract(offer.contractId, session.user.id, true)
        : null;
            ...
            <div className="border-b px-6 py-10 md:px-10 md:py-12">
                ...
                <dl className="grid max-w-4xl gap-6 sm:grid-cols-2 lg:grid-cols-3">
                    ...
                    <Fact label="Vertrag">
                        {contract ? (
                            <Link
                                href={`/account/admin/contracts/${contract.contract.id}`}
